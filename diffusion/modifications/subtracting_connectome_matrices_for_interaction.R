...
#specify directory
#input_directory <- 'V:/Archive/NECTAR_data/LENORE/derivatives/groups/F2/diff_data/longitudinal/connectome/dkFiles/weighted/fs_default_ordered/thresholded_connectomes/C&SCD/'
...
#output_directory <- 'V:/Archive/NECTAR_data/LENORE/derivatives/groups/F2/diff_data/longitudinal/connectome/dkFiles/weighted/fs_default_ordered/interaction_study(F2-F0)_C&SCD_thresholded/'
...
for (i in 1:length(all_csvs_files)/2) {
  ...
  #PAR_NAME <- substr(list.files()[j],4,16)
  PAR_NAME <- substr(list.files()[j],17,29)
